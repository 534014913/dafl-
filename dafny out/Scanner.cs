...
public class Scanner {
  ...
  static Scanner() {
    start = new Hashtable(128);
		...
		for (int i = 49; i <= 57; ++i) start[i] = 69;
		start[97] = 70; 
		start[98] = 71; 
		start[39] = 72; 
		start[48] = 73; 
...
//  [NotDelayed]
  public Scanner (string/*!*/ fullFilename, string/*!*/ fileName, Errors/*!*/ errorHandler, bool useBaseName = false, Token firstToken = null) : base() {
    Contract.Requires(fileName != null);
    Contract.Requires(errorHandler != null);
    this.errorHandler = errorHandler;
    if(firstToken.Filename == null || firstToken.Filename == "") {
      firstToken.Filename = fullFilename;
    }
...
//  [NotDelayed]
  public Scanner (Stream/*!*/ s, Errors/*!*/ errorHandler, string/*!*/ fullFilename, string/*!*/ fileName, int lineNumber = 1, bool useBaseName = false, Token firstToken = null) : base() {
    Contract.Requires(s != null);
    Contract.Requires(errorHandler != null);
    Contract.Requires(fileName != null);
    if(firstToken.Filename == null || firstToken.Filename == "") {
...
//      if (ch == EOL) { line++; col = 0; }
      ...
      while (true) {
        ...
        if (ch == EOL) {
          ...
          this.errorHandler.SemErr(ParseErrors.ErrorId.sc_malformed_pragma, Filename, prLine, prColumn, "Malformed (#line num [filename]) pragma: #" + hashLine);
          continue;
          }

          this.errorHandler.SemErr(ParseErrors.ErrorId.sc_unknown_pragma, Filename, prLine, prColumn, "Unrecognized pragma: #" + hashLine);
          continue;
        }
        return;
        }
  ...
  public void CheckLiteral() {
		switch (t.val) {
			...
			case "opaque": t.kind = 51; break;
			case "inductive": t.kind = 52; break;
			case "twostate": t.kind = 53; break;
			case "copredicate": t.kind = 54; break;
			case "lemma": t.kind = 55; break;
			case "static": t.kind = 56; break;
			case "import": t.kind = 57; break;
			case "export": t.kind = 58; break;
			case "class": t.kind = 59; break;
			case "trait": t.kind = 60; break;
			case "datatype": t.kind = 61; break;
			case "codatatype": t.kind = 62; break;
			case "var": t.kind = 63; break;
			case "const": t.kind = 64; break;
			case "newtype": t.kind = 65; break;
			case "type": t.kind = 66; break;
			case "iterator": t.kind = 67; break;
			case "method": t.kind = 68; break;
			case "colemma": t.kind = 69; break;
			case "constructor": t.kind = 70; break;
			case "modifies": t.kind = 71; break;
			case "reads": t.kind = 72; break;
			case "requires": t.kind = 73; break;
			case "ensures": t.kind = 74; break;
			case "ghost": t.kind = 75; break;
			case "provides": t.kind = 76; break;
			case "reveals": t.kind = 77; break;
			case "extends": t.kind = 78; break;
			case "new": t.kind = 79; break;
			case "nameonly": t.kind = 80; break;
			case "older": t.kind = 81; break;
			case "witness": t.kind = 82; break;
			case "reveal": t.kind = 100; break;
			case "expect": t.kind = 101; break;
			case "include": t.kind = 106; break;
			case "abstract": t.kind = 107; break;
			case "module": t.kind = 108; break;
			case "refines": t.kind = 109; break;
			case "opened": t.kind = 110; break;
    ...
    switch (state) {
      ...
      case 0: {
        ...
        t.kind = recKind; break;
			...
			case 22:
				if (ch >= 56320 && ch <= 57343) {AddCh(); goto case 21;}
				else {goto case 0;}
			case 23:
			...
			case 31:
				{t.kind = 22; break;}
			case 32:
				if (ch <= 9 || ch >= 11 && ch <= 12 || ch >= 14 && ch <= '!' || ch >= '#' && ch <= '[' || ch >= ']' && ch <= 65535) {AddCh(); goto case 32;}
			...
			case 41:
				if (ch <= '!' || ch >= '#' && ch <= 65535) {AddCh(); goto case 41;}
			...
			case 59:
				{t.kind = 96; break;}
			case 60:
				if (ch == 'n') {AddCh(); goto case 61;}
				else {goto case 0;}
			case 61:
				if (ch <= '&' || ch >= '(' && ch <= '/' || ch >= ':' && ch <= '>' || ch == '@' || ch >= '[' && ch <= '^' || ch == '`' || ch >= '{' && ch <= 65535) {apx++; AddCh(); goto case 62;}
				else {goto case 0;}
			case 62:
				{
					tlen -= apx;
			...
			case 69:
				...
				else if (ch == '_') {AddCh(); goto case 81;}
				else if (ch == '.') {AddCh(); goto case 18;}
				else {t.kind = 2; break;}
			case 70:
				recEnd = pos; recKind = 1;
				if (ch == 39 || ch >= '0' && ch <= '9' || ch == '?' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'q' || ch >= 's' && ch <= 'z') {AddCh(); goto case 2;}
				else if (ch == 'r') {AddCh(); goto case 82;}
				else {t.kind = 1; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 71:
				recEnd = pos; recKind = 1;
				if (ch == 39 || ch >= '0' && ch <= '9' || ch == '?' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'u' || ch >= 'w' && ch <= 'z') {AddCh(); goto case 10;}
				else if (ch == 'v') {AddCh(); goto case 83;}
				else {t.kind = 1; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 72:
				recEnd = pos; recKind = 1;
				if (ch >= '0' && ch <= '9' || ch == '?' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); goto case 84;}
				else if (ch == 39) {AddCh(); goto case 85;}
				else if (ch <= 9 || ch >= 11 && ch <= 12 || ch >= 14 && ch <= '&' || ch >= '(' && ch <= '/' || ch >= ':' && ch <= '>' || ch == '@' || ch == '[' || ch >= ']' && ch <= '^' || ch == '`' || ch >= '{' && ch <= 55295 || ch >= 57344 && ch <= 65535) {AddCh(); goto case 21;}
				else if (ch >= 55296 && ch <= 56319) {AddCh(); goto case 22;}
			...
			case 77:
				recEnd = pos; recKind = 1;
				if (ch == 39 || ch >= '0' && ch <= '9' || ch == '?' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); goto case 77;}
				else {t.kind = 1; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 78:
			...
			case 79:
				if (ch == '"' || ch == 39 || ch == '0' || ch == 92 || ch == 'n' || ch == 'r' || ch == 't') {AddCh(); goto case 32;}
				else if (ch == 'u') {AddCh(); goto case 33;}
				else if (ch == 'U') {AddCh(); goto case 37;}
				else {goto case 0;}
			case 80:
			...
			case 82:
				...
				else if (ch == 'r') {AddCh(); goto case 86;}
				else {t.kind = 1; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 83:
				recEnd = pos; recKind = 1;
				if (ch == 39 || ch == '?' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); goto case 11;}
				else if (ch >= '1' && ch <= '9') {AddCh(); goto case 87;}
				else if (ch == '0') {AddCh(); goto case 88;}
				else {t.kind = 1; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 84:
				recEnd = pos; recKind = 1;
				if (ch >= '0' && ch <= '9' || ch == '?' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); goto case 89;}
				else if (ch == 39) {AddCh(); goto case 90;}
				else {t.kind = 1; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 85:
				recEnd = pos; recKind = 1;
				if (ch >= '0' && ch <= '9' || ch == '?' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); goto case 89;}
			...
			case 87:
				...
				if (ch == 39 || ch == '?' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); goto case 77;}
			...
			case 122:
				...
				else if (ch == '=') {AddCh(); goto case 128;}
			...
			case 125:
				...
				else if (ch == 'i') {AddCh(); goto case 60;}
